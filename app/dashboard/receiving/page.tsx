...
export default async function ReceivingPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {purchases.length === 0 ? (
        <p className="text-gray-500">No hay recepciones todavía.</p>
      ) : (
        <div className="rounded-md border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr><th className="text-left p-3">Referencia</th><th className="text-left p-3">Proveedor</th><th className="text-left p-3">Estado</th><th className="text-right p-3">Acciones</th></tr>
            </thead>
            <tbody>
              {purchases.map((p) => (
                <tr key={p.id} className="border-t">
                  ...
                  <td className="p-3 text-right">
                    <Link href={`/dashboard/receiving/${p.id}`}>
